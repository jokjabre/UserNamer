...
(function(dice) {
    ...
    this.create_dice_materials = function(face_labels, size, margin) {
        function create_text_texture(text, color, back_color) {
			...
			context.wrapText(text, canvas.width / 2, canvas.height * 0.3, pt * 0.75);
        ...
        for (var i = 0; i < face_labels.length; ++i)
            materials.push(new THREE.MeshPhongMaterial($t.copyto(this.material_options,
                        { map: create_text_texture(swap_numbers(face_labels[i]), this.label_color, this.dice_color) })));
        return materials;
    }
    ...
    this.known_types = ['d6'];//, 'd8', 'd10', 'd12', 'd20', 'd100'];
